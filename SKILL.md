...
description: 生成确定性的 ETF 底层资产穿透指标表，覆盖 A 股上市 ETF、港股/H 股 ETF、美股成分 QDII ETF、美股上市 ETF，以及 ETF 组合穿透；输出股息率、PE、PB、收益/风险指标、持仓结构、行业主题、估值分层、重合度、PCF 质量、缓存清单和约束检查。
---
...
`selected_etf_lookthrough.py` 会生成：
...
- `profit_quality.csv`、`dividend_quality.csv`：质量摘要和覆盖率说明。
...
- `--cache` / `--no-cache`：是否在 `cache/` 写入本次持仓快照；默认开启。
...
- 组合收益/风险：用组合后的 ETF 历史曲线重新计算，不把单只 ETF 指标简单加权。
- 三年收益：历史窗口不足时留空。
